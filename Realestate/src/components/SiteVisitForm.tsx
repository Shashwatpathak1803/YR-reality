...
export function SiteVisitFormCard({ onBooked, plain = false }: { onBooked?: () => void; plain?: boolean }) {
  ...
  if (sent) {
    return (
      <div className={plain ? "py-6 text-center" : "glass rounded-3xl p-8 sm:p-10 shadow-elegant text-center"}>
        <div className="w-14 h-14 mx-auto rounded-full bg-foreground text-background grid place-items-center text-2xl">✓</div>
        <h3 className="mt-4 font-display font-bold text-2xl">Visit Scheduled!</h3>
  ...
  return (
    <form onSubmit={onSubmit} className={plain ? "" : "glass rounded-3xl p-6 sm:p-7 shadow-elegant"}>
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <label className="block">
          <span className="text-xs font-semibold text-muted-foreground">Preferred Time</span>
          <select
            value={time}
            onChange={(e) => setTime(e.target.value)}
            className="mt-1.5 w-full h-11 rounded-xl border border-input bg-white/70 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
          >
      ...
      <button
        type="submit"
        disabled={sending}
        className="mt-5 w-full inline-flex items-center justify-center gap-2 h-12 rounded-xl bg-foreground text-background font-semibold hover:bg-foreground/90 transition disabled:opacity-70"
      >
    ...
    <section id="visit" className="py-20 sm:py-28">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          viewport={{ once: true }}
          className="relative overflow-hidden rounded-[2rem] p-8 sm:p-12 bg-[image:var(--gradient-primary)] shadow-elegant"
        >
          <div className="absolute -right-24 -top-24 w-80 h-80 rounded-full bg-white/10" />
          <div className="absolute -left-16 -bottom-24 w-72 h-72 rounded-full bg-accent/30 blur-3xl" />

          <div className="relative grid lg:grid-cols-2 gap-10 items-center">
            <div className="text-primary-foreground">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass-dark text-xs font-semibold">
